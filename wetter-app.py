...
class GUI:
    def __init__(self):
        ...
        self.fenster.grid_columnconfigure(1, minsize=400)   # Rechte Spalte (Wetterwarnungen), immer 400 Pixel breit
        ...
        self.label_warnMeldung = Label(self.rechte_seite, text="Text", font=("Segoe UI Semibold", 30), bg="#404040", fg="white")
        self.label_warnMeldung.pack(padx=10, pady=100)
        ...
        # Die Schriftgröße relativ zur Fenstergröße skalieren
        self.fenster.bind("<Configure>", lambda event: self.schriftgröße_anpassen())       
        
        
    def schriftgröße_anpassen(self):
        # Hole die Fensterbreite
        fensterBreite = self.fenster.winfo_width()
        
        # Rechne die Schriftgröße aus
        bild_fontSize = int(fensterBreite * 0.15)  # Adjust the multiplier as needed
        temperatur_fontSize = int(fensterBreite * 0.08)  # Adjust the multiplier as needed

        # Die Schriftgröße für das Label aktualisieren
    ...
    def icon(self, iconDaten):
        self.label_bild.config(text=f"{iconDaten}")
        
    def warnMitteilung(self, warnDaten):
        self.label_warnMeldung.config(text=f"{warnDaten}")
...
class WetterApp:
    ...
    def hole_wetter(self):
        ...
        emoji = self.hole_emoji(iconDaten)
        self.gui.icon(emoji)

        if temperaturDaten >= 30.0:
            warnung = "Extreme Hitze! \nTrinken Sie ausreichend und \nvermeiden sie unbeschattete Orte"
        elif temperaturDaten <= -10.0:
            warnung = "Extreme Kälte! \nBekleiden Sie sich ausreichend und \nhalten Sie sich weitgehend in Innenräumen auf"    
        else:
            warnung = "Derzeit liegt keine \nGefahrensituation vor"
        
        self.gui.warnMitteilung(warnung)
