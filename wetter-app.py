...
class GUI:
	def __init__(self):
		...
		self.label_warnungsHinweis = Label(self.rechte_seite, text="ZZt. liegt keine Gefahrensituation vor", font=("Segoe UI Semibold", 20), bg="#404040", fg="white")
		self.label_warnungsHinweis.pack(anchor="w", padx=10, pady=100)
	...
	def meldung(self,warnMeldung):
        self.label_warnungsHiweis.config(text=f"{warnMeldung}")
...
class WetterApp:
	...
	def hole_wetter(self):
		...
		emoji = self.holeEmoji(iconDaten)
		self.gui.icon(emoji)
		
		warnMeldung = wetter_daten["current_weather"]["temperature"]
		warnung = self.holeWarnung(warnMeldung)
		self.gui.gefahrenMitteilung(warnung)
	...
	def holeWarnung(self, warnMeldung):
        
        if warnMeldung >= 35.0:
            warnung = "Extrem hohe Temperaturen!\n Trinken Sie ausreichend Wasser und meiden\n Sie unbeschattete Plätze"
        else:
            warnung = "Zzt. liegt keine Gefahrensituation vor"
